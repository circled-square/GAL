...
namespace gl {
    ...
    void vertex_array::specify_attrib(uint attrib_index, uint offset, uint type_id, uint size, uint vao_vbo_bind_index) {
        out("specify_attrib(m_vao=%\tattrib_index=%\toffset=%\ttype_id=%\tsize=%\tvao_vbo_bind_index=%)", m_vao, attrib_index, offset, type_id, size, vao_vbo_bind_index);
    ...
    void vertex_array::specify_attribs(const vertex_layout& layout) {
            ...
            if(layout.vertex_size != 0) { //layout.vertex_size is only optionally set
                ...
                for(auto& vbo : m_vbos) {
                    ...
                    out("stride_sum += %", vbo.get_stride());
                }
                out("layout.vertex_size = %", layout.vertex_size);
