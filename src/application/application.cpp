...
#include <internal/graphics/init.hpp>
...
namespace gal::application {
    application::application(glm::ivec2 res, const std::string &title, scene *active_scene) : m_window(res, title), m_active_scene(active_scene) {
        ...
        m_window.set_key_cb([] (GLFWwindow* window_handle, int key, int scancode, int action, int mods) {
            ...
            app->m_active_scene->raise_key_press_event(key, scancode, action, mods);
        });
        m_window.set_resize_cb([] (GLFWwindow* window_handle, int w, int h) {
            application* app = reinterpret_cast<application*>(window::window::get_user_ptr(window_handle));

            app->m_active_scene->raise_resize_event({w,h});
        });
    }
    ...
    void application::run() {
        ...
        while (!m_window.should_close() && !ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Escape))) {
            ...
            m_active_scene->render(m_window.get_framebuf_size());
